...
class ListDatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun insertNote(note: Note) { // Renamed parameter and function
    ...
    fun getllNotes(): List<Note> { // Specify the return type as List<Note>
        ...
        while (cursor.moveToNext()) {
            ...
            val note = Note(id, title, content) // Create Note objects
            noteList.add(note)
        }
        cursor.close()
        db.close()
